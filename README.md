...
- 대소문자, 연속 공백, 마지막 세미콜론을 무시한 간단한 정답 비교
- 정답 및 오답 결과 표시
- 시스템 설정과 관계없이 고정 다크 테마 적용
- 로딩, 콘텐츠, 오류 상태를 분리한 Compose UI 구조

현재 문제와 정답은 앱 내부의 샘플 데이터를 사용합니다. SQL을 실제 데이터베이스에서 실행하지 않으므로, 같은 결과를 만드는 다른 형태의 SQL까지 정답으로 판단하지는 않습니다.
...
└── ui/
    ...
    ├── SqlAnswerEvaluator.kt       # SQL 문자열 정규화와 채점
...
SQL 정답 비교를 포함한 로컬 단위 테스트를 실행합니다.
